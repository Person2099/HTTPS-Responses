...
class Variables {
    ...
    var RCC: Int = 100 // Response Code Current
}




//MARK: Possibly Useful
/*class Stuff: ObservableObject {
@EnvironmentObject var crnt: CRNT
let stuffCurrent = CRNT().$current

@Published var currentSTR: String //= String(stuffCurrent)
@Published var currentARR: [Int] //= Array(arrayLiteral: crnt.current)
//var currentCol: Color = Variables().RCColour[currentARR[0]]!
@Published var simpleDef: String //= Variables().RCRelation[crnt().current]?.simplifiedDef ?? "AN ERROR HAS OCCURED PLEASE CONTACT THE HTTP/S LOOKUP DEVELOPMENT TEAM"
@Published var detailedDef: String //= Variables().RCRelation[crnt().current]?.detailedDef ?? "AN ERROR HAS OCCURED PLEASE CONTACT THE HTTP/S LOOKUP DEVELOPMENT TEAM"
@Published var type: String //= Variables().RCRelation[crnt().current]!.responseCodeType
}*/
